...
export async function analyzeBundle(
  imageUrls: string[],
  mode: "single_lot" | "catalogue",
  language: "en" | "fr" | "es",
  currency: string
): Promise<{ lots: any[]; summary?: string; language?: "en" | "fr" | "es"; currency?: string }> {
  if (!Array.isArray(imageUrls) || imageUrls.length === 0) {
    return { lots: [], language, currency };
  }
  const systemPrompt = getAssetSystemPrompt(mode as any, language, currency);
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: systemPrompt },
      {
        role: "user",
        content: [
          { type: "text", text: `Grouping mode: ${mode}. Analyze these images and return valid JSON as instructed.` },
          ...
          ...imgs.map(({ base64, mime }) => ({ type: "image_url" as const, image_url: { url: `data:${mime};base64,${base64}` } })),
        ],
      },
    ],
  ...
  const parsed = JSON.parse(content) as { lots?: any[]; language?: any; currency?: any; summary?: string };
